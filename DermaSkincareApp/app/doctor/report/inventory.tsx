...
export default function InventoryReportScreen() {
  ...
  const categoryBreakdown = useMemo(() => {
    const raw = data?.category_breakdown || {};
    return Object.entries(raw).map(([category, info]) => ({
      category,
      ...info,
    }));
  }, [data]);
    ...
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 60 }}>
      ...
      <View style={styles.statsGrid}>
        <View style={styles.statCard}>
          ...
          <Text style={styles.statValue}>{data?.summary?.total_items || 0}</Text>
      ...
      {categoryBreakdown.length > 0 && (
        <>
          ...
          {categoryBreakdown.map((cat, idx) => {
            const maxQty = Math.max(...categoryBreakdown.map((c) => c.quantity));
            return (
              <View key={idx} style={styles.categoryRow}>
                ...
                <View style={styles.categoryBarContainer}>
                  <View
                    style={[
                      styles.categoryBar,
                      { width: `${(cat.quantity / maxQty) * 100}%` },
                    ]}
                  />
                </View>
                <Text style={styles.categoryCount}>
                  {cat.count} items ({cat.quantity})
                </Text>
              </View>
            );
          })}
            ...
            <View key={it.id} style={styles.row}>
              <View style={styles.rowLeft}>
                ...
                <Text style={styles.itemMeta}>
                  Min level: {it.reorder_level ?? "-"} • Current: {it.quantity}
                </Text>
              </View>
              ...
              <View key={tx.id} style={styles.row}>
                ...
                <View style={styles.rowRight}>
                  <Text
                    style={[
                      styles.itemStock,
                      { color: tx.type === "IN" ? "#28a745" : "#dc3545" },
                    ]}
                  >
                    {tx.type === "IN" ? "+" : "-"}
                    {tx.quantity}
